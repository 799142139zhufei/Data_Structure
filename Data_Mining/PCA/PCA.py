...
class PCA_model(object):
    ...
    def PCA_2d_example(self):
        '''二维降为一维主运行函数'''
        '''加载数据并作图'''
        data = spio.loadmat('data.mat')
        X = data['X']
        plt = self.plot_data_2d(X, 'bo')
        plt.axis('square')
    ...
    def plot_data_2d(self, X, marker):
        # 可视化二维数据
        plt.plot(X[:, 0], X[:, 1], marker)
        return plt

    def display_imageData(self, imgData):
        # 显示图片
        sum = 0
        '''
